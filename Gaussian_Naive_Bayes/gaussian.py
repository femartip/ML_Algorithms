...
def pxc(pc, mu, sigma, X):
    res = np.empty((np.shape(X)[0], 1))        #DxN 
    ...
    Wc = -1/2 * inv    #DxD
    wc = inv @ mu         #Dim = Dx1
    wc0 = np.log(pc) - 1/2 * logdet(sigma) - 1/2 * (np.transpose(mu) @ inv @ mu)      #dim = 1  
...
def logdet(sigma):
    ...
    eigval = eigval.real
    #Compara si log es 0
    if np.any(eigval <= 0): return np.log(sys.float_info.min)  #Dx1  Vector de valores simples
    else: return np.sum(np.log(eigval))